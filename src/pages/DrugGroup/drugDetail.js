...
@connect(({ drugGroupModal, basicdata, loading }) => ({
  drugGroupModal,
  basicdata,
  loadingUpdate: loading.effects['drugGroupModal/edit'],
  loadingGet: loading.effects['drugGroupModal/getDetail'],
}))
class Drug extends PureComponent {
  ...
  submitForm = () => {
    const { form, drugGroupModal: { editFlag, id ,groupData} } = this.props;
  ...
  transfer = (props) => {
    const { drugGroupModal: { detailData } } = props || this.props;
  ...
  render() {
    ...
    let { drugGroupModal: { detailData,editFlag } } = this.props;
    if(editFlag == 0){
      detailData={}
    }
    ...
    const {drugGroupModal:{groupData}}=this.props;
